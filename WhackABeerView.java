...
public class WhackABeerView extends SurfaceView implements WhackABeerDrinks.Callback, Runnable {
    ...
    //DatabaseReference myRef = database.getReference("messagessss");
    ...
    public void gameOver(){
        try {
            ...
            //myRef.setValue("Hello, World!122");
            Date date=new Date();
            //myRef.child("ID").child("reactii").setValue(reaction_times);
            myRef.child(android_id).child(new Timestamp(date.getTime() ).toString().replace(".",",") ).child("react").setValue(reaction_times);
